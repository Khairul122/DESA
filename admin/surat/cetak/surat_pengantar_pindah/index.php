...
$id = $_GET['id'];
$qCek = mysqli_query($connect, "SELECT penduduk.*, surat_pengantar_pindah.* FROM penduduk LEFT JOIN surat_pengantar_pindah ON surat_pengantar_pindah.nik = penduduk.nik WHERE surat_pengantar_pindah.id_pp='$id'");
while ($row = mysqli_fetch_array($qCek)) {
	...
	foreach ($qTampilDesa as $rows) {
		...
		$qCekPejabatDesa = mysqli_query($connect, "SELECT pejabat_desa.jabatan, pejabat_desa.nama_pejabat_desa FROM pejabat_desa LEFT JOIN surat_pengantar_pindah ON surat_pengantar_pindah.id_pejabat_desa = pejabat_desa.id_pejabat_desa WHERE surat_pengantar_pindah.id_pejabat_desa = '$id_pejabat_desa' AND surat_pengantar_pindah.id_pp='$id'");
			...
			<body>
				<div>
					<table width="100%">
						...
						<br>
						<div align="center"><u>
								<h4 class="kop">SURAT PENGANTAR PINDAH</h4>
							</u></div>
						<div align="center">
					...
					<div id="isi3">
						...
						<table width="100%" style="text-transform: capitalize;">
							...
							<tr>
								<td class="indentasi">Alamat Sekarang</td>
								<td>:</td>
								<td><?php echo $row['jalan'] . ", RT" . $row['rt'] . "/RW" . $row['rw'] . ", Desa " . $row['desa'] . ", Kecamatan " . $row['kecamatan'] . ", Kabupaten " . $row['kabupaten']; ?></td>
							</tr>
							<tr>
								<td class="indentasi">Alamat Tujuan Pindah</td>
								<td>:</td>
								<td style="text-transform: uppercase;"><?php echo $row['alamat_pindah']; ?></td>
							</tr>
							<tr>
								<td class="indentasi">Jumlah Keluarga Pindah</td>
								<td>:</td>
								<td style="text-transform: capitalize;"><?php echo $row['keperluan']; ?> (orang)</td>
							</tr>
						</table>
						<br><br>
						<table width="100%">
							<tr>
								<td class="indentasi">Adapun Permohonan Pindah Penduduk WNI yang bersangkutan sebagai mana terlampir.</a></u></b></td>
							</tr>
						</table><br>
						<table width="100%">
							<tr>
